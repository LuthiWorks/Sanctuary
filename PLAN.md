...
- **Status / remaining work.** §1–§5 committed; §4 (async actor/learner) built and approved. **Remaining:** §6 (staleness — `advance_root` must refresh `_context_latents`/`_target_positions`, `mcts.py:152-171`; full wiring in the tracked spec below) and the **wake/sleep learner build** (parameters + open questions recorded above; still needs a spec pass).
