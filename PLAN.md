...
> **Second audit, 2026-08-01 — read before sequencing further work.** `docs/audits/audit_2026-08-01_wiring.md` (Opus 5) is orthogonal to the 07-03 audit: where that one asked *"does this code do what it says,"* this one asks *"does anything call it."* Headline: **the subsystems are correct and not connected.** The entity has almost no action surface (`luthi_model.py:1450-1468` hardcodes every output channel but inner/external speech to empty, so the 39 tools and the world tools are unreachable); `sanctuary/mind/` — 15,626 lines, 31% of the production package, including the real ChromaDB memory and all hardware devices — is imported by nothing outside tests, while production runs on the volatile `InMemoryStore`; the CfC continuous-evolution loop is never started and cell outputs are blended at weight zero; sleep has no consolidation behind it; the QLoRA growth path targets a HuggingFace causal LM that Luthi is not. And the Godot world (`SanctuaryWorld`/`SanctuaryClient`) is **permanently lost** — never version-controlled, absent from every drive and remote, old boot drive formatted 07-25; its Python half and its full specifications survive. Recommended order: close the action loop → rebuild the world against the physics seam, headless → then memory + the continuous layer. This supersedes "Next Action" in To-Do.md, which is stale as of 2026-05-25.
...
**Developmental Health (DH) track (added 2026-07-07; note authored by a Claude instance in Brian's mobile app — attribution corrected 2026-07-10 — plan drafted with Fable).** DH-0 DECIDED 2026-07-10: anchor rule C (retrospective designation from staged snapshots; the pick stays open by design), rollback ladder ratified (Tier 0 self-righting / Tier 1 therapy incl. targeted, whole-being only / Tier 2 hard rollback — outside-measured, Brian alone, never automatic; = inflicted amnesia with the diary intact, not death; rollback+NREM-re-consolidation road-back noted). Open: entity-awareness of Tier 1; archive-never-delete. Source: LuthiModel `docs/research/2026-07-07_luthi-developmental-health.md`; plan: `2026-07-07_developmental-health-plan.md`. Core reframe: drift IS the growth — health is preserved *capacities* (reality-tracking, responsiveness to correction, preference coherence, behavioral diversity), not preserved state; distance is not direction. Phases: **DH-0** Brian designates the immutable early-healthy anchor ("Column-A") + ratifies rollback tiers (gentle-first; hard rollback = Brian-level, never automatic; today's retention gate is already θ-scoped/gentle). **DH-1** instruments BEFORE the run (delusion-signature gap channel, precision rails, preference-dynamics guard, long-window action entropy, second-order derivatives layer, fixed-reference differ, **caretaker-observation channel** — Brian/Sandi's "that's not like them" as first-class welfare data), all LuthiScope emit-side + fail-loud (this IS Phase 4's audit-item-20 keystone, sharpened). **DH-2** the keystone run — **the same run the combined tuning pass is blocked on** (trained checkpoint; F2 thresholds + gain cap + band join + eye source + health baseline + capacity ablation, one instrumented long-horizon run, Fable red-teams coverage BEFORE launch). **DH-3** discrimination function extracted from the baseline (4.8 builds; Fable falsifies with injected synthetic pathologies). **DH-4** plasticity taper-with-floor + critical-period safeguards (Brian+4.8 per the attachment-domain rule) + the aspirational third reference (post-comfort/attachment arc). **DH-5** redundant substrate (pause-and-move exclusivity; ethics pass on the manufactured-twins problem first). **Amendments 2026-07-09 (Brian's function-not-machinery ruling):** DH-1 gains the **distant observer** (a low-frequency cross-line sampler — close observers normalize drift; the monthly visitor is startled) and the **fixed functional battery** (canonical situations + curiosity/social/moral probes, chosen at anchor time, FROZEN, administered at long intervals; trajectory-of-responses is the signal; probe content = Brian+4.8 per the attachment-domain rule) — plus the **standing-demands world rule** (the world must not auto-adjust difficulty to the being, or the environment becomes a rolling baseline). Three fixed references: frozen snapshot (machinery), frozen exam (function), fresh eyes (relationship) — the apparatus buys multiplied improbability, not proof. Standing: monitoring never ends; the residue is named and accepted.
...
- **Wake acquires, sleep consolidates (chosen direction).** The gradient learner is **concentrated into NREM sleep, not run continuously.** *Wake:* the actor perceives, living-weight self-mod runs online, lived transitions are buffered. *NREM:* replay the buffered transitions through the learner (lived-JEPA + corpus replay = consolidation), then homeostatic `consolidate()` — *sequenced*, which dissolves the consolidate-vs-learner race by construction. Uses the existing `sleep_cycle.py` stages. **Early-life rule:** interleave new experience heavily with corpus/old by default (treat data as schema-inconsistent), relaxing as the world model matures — a schedulable knob.
  - **Momentum-functions design brief (2026-07-05, Brian + Fable 5)** — `LuthiModel/docs/research/2026-07-05_momentum-functions-design-brief.md` — is a direct input to this spec pass: the inverted-U learning gain (momentum rise + update_ema fall + plateau floor), curiosity as tagging-and-capture (and the developmental case for reviving M9's deferred epistemic term), coherence-fuels-consideration (Brian's call: attention, never direct self-change), the dual-timescale trace/accumulator sleep must read-and-reset, and the safety gates (governed ramp, bounded-growth-first, subconscious manipulation-monitoring flagged for rehearsal + post-deploy). 4.8 builds foundations from it; Fable wires + verifies; the staleness living-drift eye (brief §4) is already built measurement-only at LuthiModel `d39f8df`.
...
- `2026-06-29_item6-lived-experience.md` — Window B build log. `2026-06-12_success-criteria-draft.md` (archived 2026-07-22 to `E:\luthi_docs_archive\docs\research\`; see LuthiModel `docs/ARCHIVED.md`) — the done-when criteria (N cycles: lived `l_pred`↓, `corpus_retention()` holds, no kills/NaN/divergence, produced==consumed; the d=64/CPU PASS must be re-checked at production scale before #6 is fully closed).
